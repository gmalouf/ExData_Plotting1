...
constructPlot <- function(uciDataSet, workDirectory) {
  png(filename = paste(workDirectory, 'plot3.png', sep = '/'))
  with(uciDataSet, plot(Time, Sub_metering_1, type = 'l', xlab = '', 
                        ylab = 'Energy sub metering'))
  with(uciDataSet, lines(Time, Sub_metering_2, col = 'red'))
  with(uciDataSet, lines(Time, Sub_metering_3, col = 'blue'))
